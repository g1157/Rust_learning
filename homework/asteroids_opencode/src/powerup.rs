...
pub const WEAPON_SPAWN_MIN: f64 = 10.0;
pub const WEAPON_SPAWN_MAX: f64 = 20.0;
...
pub fn spawn(now: f64, powerups: &mut Vec<PowerUp>, next_spawn: &mut f64, next_weapon_spawn: &mut f64) {
    // 生成护盾道具
    if now >= *next_spawn {
    ...
    if now >= *next_weapon_spawn {
        ...
        *next_weapon_spawn = schedule_next_weapon_spawn(now);
    }
...
pub fn schedule_next_weapon_spawn(now: f64) -> f64 {
    now + rand_seconds(WEAPON_SPAWN_MIN, WEAPON_SPAWN_MAX)
}
...
fn draw_dual_shot_icon(center: Vec2, alpha: f32) {
    ...
    // 向上箭头
    draw_line(center.x, center.y, center.x, center.y - arrow_len, 3., color);
    draw_line(center.x, center.y - arrow_len, center.x - 5.0, center.y - arrow_len + 8.0, 3., color);
    draw_line(center.x, center.y - arrow_len, center.x + 5.0, center.y - arrow_len + 8.0, 3., color);
