...
1. 用 Metropolis 蒙特卡洛方法模拟二维伊辛模型，计算磁化强度并估算临界指数 $\beta$
2. 比较正方形与三角形晶格的结果，验证临界指数的**普适性**
3. 在高温极限检验 $M(H) \approx \tanh(H/T)$，分析偏差与温度的关系
...
**临界慢化**：在 $T \approx T_c$ 附近，系统自相关时间发散，需要更长的平衡时间与采样。

### 2.5 高温外场响应

在 $T \gg T_c$ 时，忽略自旋关联，单自旋平均：
...
扫描不同的 $\beta^*$ 值（0.05 到 0.20），计算 $M^{1/\beta^*}$ vs $T$ 的线性拟合 $R^2$，选择 $R^2$ 最大的 $\beta^*$ 作为最佳估计。
...
*图1: 正方形晶格磁化强度随温度变化曲线，红色虚线标注理论临界温度 $T_c \approx 2.269$*
...
| 温度 $T$ | $M_{\text{sim}}$ | $M_{\text{theory}} = \tanh(H/T)$ | 绝对误差 |
|:--------:|:----------------:|:--------------------------------:|:--------:|
...
**结论**：随着温度降低，与 $\tanh(H/T)$ 的偏差显著增大，从 T=100 到 T=10，误差增大约 **40 倍**。这验证了自旋相互作用在低温时不可忽略。
...
| 物理量 | 实验值 | 理论值 | 相对误差 |
|:------:|:------:|:------:|:--------:|
| $\beta$ (正方形) | 0.118 | 0.125 | 5.6% |
| $\beta$ (三角形) | 0.120 | 0.125 | 4.0% |
| $T_c$ (正方形) | — | 2.269 | — |

### 5.2 普适性验证

正方形和三角形晶格得到**相同的临界指数** $\beta \approx 0.125$，尽管它们的：
- 临界温度不同（2.27 vs 3.64）
- 配位数不同（4 vs 6）
- 晶格几何不同

这验证了**普适性**的核心概念：临界指数只依赖于系统的维度（2D）和对称性（$Z_2$），而非微观细节。

### 5.3 误差来源分析

| 误差来源 | 影响 | 改进方法 |
|:--------:|:----:|:--------:|
| **有限尺寸效应** | 相变变"圆滑"，$\beta$ 略偏 | 更大晶格，有限尺寸标度 |
| **统计涨落** | 测量值有方差 | 增加 sweeps，多次独立运行 |
| **临界慢化** | $T \approx T_c$ 时平衡化慢 | Wolff/Swendsen-Wang 团簇算法 |
| **$T_c$ 估计误差** | log-log 拟合敏感 | Binder 比方法确定 $T_c$ |

### 5.4 高温近似的物理解释

$M = \tanh(H/T)$ 是**单自旋近似**的结果，假设自旋之间独立。

- **$T = 100$**（约 44 倍 $T_c$）：热能远大于相互作用能，$k_B T \gg J$，自旋几乎独立
...
2. **并行化**：多温度并行模拟
3. **有限尺寸标度**：多个 $L$ 值外推 $L \to \infty$

### 6.2 其他物理量

1. **比热** $C = \frac{\langle E^2 \rangle - \langle E \rangle^2}{k_B T^2}$
2. **磁化率** $\chi = \frac{\langle M^2 \rangle - \langle M \rangle^2}{k_B T}$
3. **Binder 比** $U = 1 - \frac{\langle M^4 \rangle}{3\langle M^2 \rangle^2}$
...
1. **临界指数**：估算得到 $\beta \approx 0.12$，与 Onsager 精确解 $\beta = 0.125$ 符合良好（误差 < 6%）

2. **普适性**：正方形和三角形晶格给出**相同的临界指数**，验证了相变理论的普适性概念

3. **高温近似**：验证了 $T \gg T_c$ 时 $M(H) \approx \tanh(H/T)$ 的近似，以及随温度降低偏差增大的物理规律
...
| 公式 | 描述 |
|:----:|:----:|
| $H = -J \sum_{\langle i,j \rangle} s_i s_j - h \sum_i s_i$ | 哈密顿量 |
| $M = \frac{1}{N} \sum_i s_i$ | 磁化强度 |
| $M \sim (T_c - T)^\beta$ | 临界幂律 |
| $\beta = 1/8 = 0.125$ | 二维临界指数 |
| $\Delta E = 2 s_i (\sum_{nn} s_j + h)$ | 翻转能量差 |
| $p = \min\{1, e^{-\Delta E/T}\}$ | Metropolis 接受率 |
| $M_{\text{theory}} = \tanh(h/T)$ | 高温单自旋近似 |
