...
「普适性」（Universality）是相变理论的核心概念：临界指数只依赖于系统的维度和对称性，与晶格细节（正方形、三角形）无关。
...
> ★ 关键发现：从 $T = 100$ 到 $T = 10$，偏差增大约 40 倍，验证了自旋相互作用的影响随温度降低而增大
...
2. 普适性验证：正方形和三角形晶格给出相同的临界指数，尽管它们的临界温度（$2.27$ vs $3.64$）和配位数（$4$ vs $6$）不同。这验证了普适性——临界指数只依赖于维度（2D）和对称性（$Z_2$）。
